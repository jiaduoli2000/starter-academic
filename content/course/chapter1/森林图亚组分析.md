...
下面的描述是具有三个子组的森林样地的示例。个别研究的结果按其分组显示在一起。在每个子组下面，一个汇总多边形显示了将随机效果模型仅拟合到该组内的研究时的结果。图底部的汇总多边形显示了在分析所有13项研究时随机效果模型的结果。

## 图示

![](https://i.loli.net/2021/03/11/pyVrORfGcKMbUi9.png)
